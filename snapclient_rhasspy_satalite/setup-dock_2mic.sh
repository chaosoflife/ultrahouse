...
### WITH a AHT sensor
###
### This script installs the seeed voicecard onto the system
### and sets-up docker containers for rhasspy and snapclient (using a docker-compose file)
...
sudo apt-get install -y python3-full python3-pip dkms git avahi-utils mosquitto-clients portaudio19-dev libasound2 libasound2-plugins alsa-utils docker-compose libatlas-base-dev
...
### Build the docker-compose.yml for Rhasspy and Snapclient
echo "Building the docker-compose.yml for Rhasspy and Snapclient..."
cat << EOF | sudo tee /home/blueberry/docker-compose.yml > /dev/null
version: "3.7"
services:
  rhasspy:
    container_name: rhasspy
    hostname: /${HOSTNAME}
    image: "klabacha/rhasspy:armv6"
    volumes:
      - /home/blueberry/rhasspy/:/profiles
  ...
  snapclient:
    ...
    hostname: /${HOSTNAME}
    image: "klabacha/snapclient:armv6"
    volumes:
      - /etc/asound.conf:/etc/asound.conf
      - /usr/share/alsa:/usr/share/alsa
      - /etc/localtime:/etc/localtime:ro
    ...
    devices:
      - "/dev/snd:/dev/snd"
    command: ["-h", "${host_id}", "--player", "alsa", "-s", "playback" ]
    restart: unless-stopped
EOF
